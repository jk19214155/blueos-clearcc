    ...
    while (size > 0) {
        ...
        if (cache_block_index < table->block_count && (cache_block->lba * 512) == block_base && cache_block->valid != CACHE_BLOCK_IDLE) {
            if(cache_block->valid == CACHE_BLOCK_ACTIVE) {
                ...
                sprintf(cache_text_buff, "DEBUG: Request details - sbuffer: %ld, dbuffer: %p, size: %ld\n", req->sbuffer, req->dbuffer, req->size);
                cons_putstr0(task_now()->cons, cache_text_buff);
            ...
            // 缓存未命中，创建新的缓存块
            CACHE_BLOCK* cache_block_point = NULL;
            if (table->block_count == MAX_CACHES) {
                // 缓存已满，释放一个块
                EFI_STATUS status = _cache_block_rel(this, &cache_block_point);
                if(EFI_ERROR(status)) {
                    sprintf(cache_text_buff, "ERROR: Failed to release cache block - status: %d\n", status);
                    cons_putstr0(task_now()->cons, cache_text_buff);
                    return status;
                }
            } else {
            ...
            // 分配内存并初始化新的缓存块
            if(cache_block_point->buffer == NULL)
                cache_block_point->buffer = memman_alloc_page_64_4m(NULL);
            cache_block_point->buffer = memman_alloc_page_64_4m(NULL);
            cache_block_point->size = CACHE_SIZE;
            cache_block_point->lba = block_base / 512;
            cache_block_point->valid = CACHE_BLOCK_ENABLE;
            sprintf(cache_text_buff, "DEBUG: New cache block created - lba: %ld, size: %ld\n", cache_block_point->lba, cache_block_point->size);
            cons_putstr0(task_now()->cons, cache_text_buff);
            
            // 插入新的缓存块
            insert_sorted(table->cache_blocks_point, cache_block_point, table->block_count++);
            ...
            REQUEST *req = &(table->requests[request_index]);
            req->cache_blocks_point = cache_block_point;
            req->sbuffer = cache_block->buffer + block_offset;
            req->dbuffer = buff;
            req->size = copy_size;
            req->valid = CACHE_BLOCK_ACTIVE;
...
EFI_STATUS cache_sync_ahci(CACHE_TABLE *this) {
    ...
    for (int i = 0; i < MAX_CACHES; i++) {
        if (this->cache_blocks[i].valid != CACHE_BLOCK_IDLE) {
            sprintf(cache_text_buff, "cache_block %d: valid=%ld, lba=%ld, size=%ld\n", 
    ...
    for (int i = 0; i < MAX_REQUESTS; i++) {
        if (this->requests[i].valid != CACHE_BLOCK_IDLE) {
            sprintf(cache_text_buff, "请求 %d: valid=%ld, size=%ld\n", 
    ...
    for (int i = 0; i < MAX_SYNC_REQUESTS; i++) {
        if (this->sync_requests[i].valid != CACHE_BLOCK_IDLE) {
            sprintf(cache_text_buff, "sync_request %d: valid=%ld, flags=%ld\n", 
	...
	sprintf(cache_text_buff,"Waiting for %d FIS transfers to complete...\n", fis_index);
	cons_putstr0(task_now()->cons,cache_text_buff);
	...
	for(int i=0;i<fis_index;i++){
		...
		if(prdbc==cache_fis_data[i]){
			sprintf(cache_text_buff,"FIS %d transfer complete: %ld/%ld bytes\n", i, prdbc, cache_fis_data[i]);
			cons_putstr0(task_now()->cons,cache_text_buff);
			continue;
		}
		else{
    ...
    for (int i = 0; i < request_count; i++) {
		...
		sprintf(cache_text_buff,"Processing request %d: Cache block status %ld, Request address 0x%lx\n", i, cache_block->valid, (unsigned long long)req);
		cons_putstr0(task_now()->cons,cache_text_buff);
        ...
        if (cache_block->valid == CACHE_BLOCK_ACTIVE) {
			...
			sprintf(cache_text_buff,"Request %d processed, Remaining requests: %d\n", i, this->request_count);
			cons_putstr0(task_now()->cons,cache_text_buff);
        }
    }
	{	int i=0 ,j=0;
