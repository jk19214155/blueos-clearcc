...
typedef struct {
    ...
    unsigned char manufactureWeek;  
    unsigned char manufactureYear;   
    unsigned char structVersion;
    unsigned char structRevision;
    unsigned char inputParameters;  
    unsigned char screenWidth;  
    unsigned char screenHeight;  
    unsigned char gamma;  
    unsigned char features;
    unsigned char colorCoordinates[10];
    unsigned char estTimings1;  
    unsigned char estTimings2; 
    unsigned char vendorTimings;
    struct {
    ...
    struct {
        ...
        unsigned char horzBlank;
        unsigned char horzActiveBlankMsb;  
        unsigned char vertActive;  
        unsigned char vertBlank;
        unsigned char vertActiveBlankMsb; 
        unsigned char horzSyncOffset;  
        unsigned char horzSyncPulse;  
        unsigned char vertSync;  
        unsigned char syncMsb;
        unsigned char dimensionWidth;  
        unsigned char dimensionHeight; 
        unsigned char dimensionMsb;  
...
struct Mode {
    struct {
        unsigned int active;
        unsigned int syncStart;
        unsigned int syncEnd;
        unsigned int total;
    } horizontal, vertical;  
    unsigned int dot;
};

struct Mode edidToMode(DisplayData edid) {
    struct Mode mode;
    
    if (edid.detailTimings[0].pixelClock == 0) {
    ...
    if (edid.detailTimings[0].horzBorder != 0 || edid.detailTimings[0].vertBorder != 0) {
        // Add error handling code here
        return mode;
    }
    
    int horz_active = edid.detailTimings[0].horzActive    
                    | ((edid.detailTimings[0].horzActiveBlankMsb >> 4) << 8);
    // Other variables defined similarly...
    
    printf("horizontal: %d, %d, %d, %d\n", horz_active, horz_blank, 
           horz_sync_offset, horz_sync_pulse);
    ...
    printf("vertical: %d, %d, %d, %d\n", vert_active, vert_blank,
           vert_sync_offset, vert_sync_pulse);
